...
for i, folder in enumerate(st.session_state['data_folders']):
    ...
    st.sidebar.write(os.path.basename(folder))
...
for i, data in enumerate(st.session_state['data_folders']):
    ax.plot(st.session_state['data'][i]["time/s"], st.session_state['data'][i]["Ewe/V"], color=st.session_state['plot_settings'][i]['color'], linestyle=st.session_state['line'][i]['line_type'], linewidth=st.session_state['linewidth'][i]['line_width'])
